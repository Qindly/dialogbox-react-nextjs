...
import { ChatRequestBody } from "@/types/Conversation";
...
export async function POST(request: NextRequest) {
  const { chats } = (await request.json()) as ChatRequestBody;
  const encoder = new TextEncoder();
  const stream = new ReadableStream({
    async start(controller) {
      const chatText = chats[chats.length - 1].text;
      for (let i = 0; i < chatText.length; i++) {
        await sleep(100);
        controller.enqueue(encoder.encode(chatText[i]));
      }
      controller.close();
    },
  });
